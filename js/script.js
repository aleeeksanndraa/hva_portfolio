  ...
  const PROJECTS = [
    {
      ...
      chapters: [
        { n: "OVERVIEW", title: "A name tag that became a keychain", text: "Made in the first week of Digital Design, when nobody in class knew each other's names yet. What started as a slick metal debit-card concept ended up as a bedazzled clear-acrylic keychain covered in rhinestones — the material changed the whole idea.", media: [{ label: "FIG. 01 — FINAL KEYCHAIN", kind: "wide" }] },
        ...
        { n: "RESEARCH", title: "Research", text: "Less research than reconnaissance: what does this lab actually have. I wanted aluminium and couldn't find any in the material stock, so the badge became a lesson in designing around what's on the shelf rather than what's in my head.", media: [{ label: "FIG. 02 — MATERIAL STOCK", kind: "small" }, { label: "FIG. 03 — LASER CUTTER BRIEFING", kind: "small" }] },
        { n: "CONCEPT", title: "Concept", text: "First idea: a slim metal card, wallet-sized, name engraved into brushed aluminium — understated, a bit like a debit card. Drawn in Figma as a card shape with a cut into one edge, doubling as a bottle opener.", media: [{ label: "FIG. 04 — FIGMA CARD SHAPE", kind: "wide" }] },
        { n: "SKETCHES", title: "Sketches", text: "The card outline, the bottle-opener notch and the lettering, all built as vectors in Figma before anything touched the laser cutter — the file the machine actually reads.", media: [{ label: "FIG. 05 — CUT FILE", kind: "tall" }] },
        { n: "EXPERIMENTS", title: "Experiments", text: "No aluminium, so clear acrylic instead. First pass out of the laser cutter: the card with \"Sasha\" engraved into it, plus a set of separate 3D letters cut from the same sheet. Held up to the light, the offcuts looked exactly like jelly sweets.", media: [{ label: "FIG. 06 — FIRST LASER-CUT PIECE", kind: "small" }, { label: "FIG. 07 — LOOSE ACRYLIC LETTERS", kind: "small" }] },
        { n: "ITERATIONS", title: "Iterations", text: "The jelly look changed the direction completely: from a slick metal card to a Y2K keychain. Raided my own nail kit for rhinestones and sparkles, bedazzled the acrylic card by hand, then built up the keychain with extra charms until it read as one deliberately messy, glamorous object.", media: [{ label: "FIG. 08 — BEDAZZLING IN PROGRESS", kind: "wide" }] },
        { n: "PRODUCTION", title: "Production", text: "Every rhinestone placed by hand, so no two badges from this batch would ever match. The 3D letters, the engraved card and the added charms were glued and wired together into one keychain.", media: [{ label: "FIG. 09 — ASSEMBLED KEYCHAIN", kind: "wide" }] },
